...
void DatagramController::ReturnFrameInfoDatagram(char *buffer, int buffer_length)
{
	...
	buffer[2] = 'M';
	buffer[3] = ':';
	buffer[4] = static_cast<char>((number_of_datagrams_ & 0xFF000000) >> 24);
	buffer[5] = static_cast<char>((number_of_datagrams_ & 0x00FF0000) >> 16);
	buffer[6] = static_cast<char>((number_of_datagrams_ & 0x0000FF00) >> 8);
	buffer[7] = static_cast<char>(number_of_datagrams_ & 0x000000FF);
	buffer[8] = static_cast<char>(((frame_number_ & 0x003F) << 2) + (sender_number_ & 0x0003));
	...
	able_to_transmit_(false)
{
	...
	number_of_datagrams_ = static_cast<unsigned int>(ceil(static_cast<float>(coded_cloud_string.size()) / static_cast<float>(buffer_length-5)));
	...
	for (int i = 0; i < number_of_datagrams_; ++i)
	{
		...
		//	printf("now");
		//}
		first = static_cast<char>((i & 0xFF000000) >> 24);
		second = static_cast<char>((i & 0x00FF0000) >> 16);
		third = static_cast<char>((i & 0x0000FF00) >> 8);
		fourth = static_cast<char>(i & 0x000000FF);
		fifth = static_cast<char>(((frame_number_ & 0x003F) << 2) + (sender_number_ & 0x0003));
		/*printf("%d %d %d %d \n", )*/
		datagram_list_[i].data_s += first;
		datagram_list_[i].data_s += second;
		datagram_list_[i].data_s += third;
		datagram_list_[i].data_s += fourth;
		datagram_list_[i].data_s += fifth;
		std::string temp;
