...
void ThorClientSystem::CloudDecoder()
{
	...
	while (1)
	{
		...
		if (actual_decoding_frame != nullptr)
		{
			...
			//fwrite(traaaa.c_str(), sizeof(char), traaaa.size(), file_ptr);
			...
			//std::string act_frame = "act_frame_" + to_string(counter) + ".pcd";
			//pcl::PointCloud<pcl::PointXYZRGBA> cloud = *decompressed_cloud;
			//pcl::io::savePCDFileASCII(act_frame, cloud);
